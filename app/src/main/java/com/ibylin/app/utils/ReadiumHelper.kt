...
/**
 * Readium 阅读器助手类
 * 负责管理 Readium 阅读器的核心功能，支持多种电子书格式
 */
@Singleton
class ReadiumHelper @Inject constructor(
    @ApplicationContext private val context: Context,
    private val config: ReadiumConfig
) {
    ...
    fun openBook(bookPath: String): Boolean {
        return try {
            android.util.Log.d("ReadiumHelper", "开始打开书籍: $bookPath")
            
            // 检查文件是否存在
            val file = java.io.File(bookPath)
            if (!file.exists()) {
                android.util.Log.e("ReadiumHelper", "文件不存在: $bookPath")
                throw Exception("文件不存在: $bookPath")
            }
            
            if (!file.canRead()) {
                android.util.Log.e("ReadiumHelper", "文件无法读取: $bookPath")
                throw Exception("文件无法读取: $bookPath")
            }
            
            android.util.Log.d("ReadiumHelper", "文件检查通过，大小: ${file.length()} 字节")
            ...
            val result = when {
                bookPath.endsWith(".epub", ignoreCase = true) -> {
                    android.util.Log.d("ReadiumHelper", "检测到EPUB文件")
                    val epubResult = openEpubBook(bookPath)
                    android.util.Log.d("ReadiumHelper", "EPUB文件处理结果: $epubResult")
                    epubResult
                }

                bookPath.endsWith(".pdf", ignoreCase = true) -> {
                    android.util.Log.d("ReadiumHelper", "检测到PDF文件")
                    openPdfBook(bookPath)
                    true
                }
                else -> {
                    android.util.Log.d("ReadiumHelper", "作为通用文档处理")
            ...
            if (result) {
                isBookLoaded = true
                android.util.Log.d("ReadiumHelper", "书籍打开成功")
                
                // 在主线程中显示阅读器视图
                android.os.Handler(android.os.Looper.getMainLooper()).post {
                    try {
                        showReaderView()
                        android.util.Log.d("ReadiumHelper", "阅读器视图已在主线程中显示")
                    } catch (e: Exception) {
                        android.util.Log.e("ReadiumHelper", "显示阅读器视图失败: ${e.message}", e)
                    }
                }
            } else {
                android.util.Log.e("ReadiumHelper", "书籍打开失败")
                isBookLoaded = false
            }
            
            result
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "打开书籍失败: ${e.message}", e)
            e.printStackTrace()
            isBookLoaded = false
            false
        }
    ...
    private fun openEpubBook(bookPath: String): Boolean {
        return try {
            android.util.Log.d("ReadiumHelper", "开始处理EPUB文件: $bookPath")
            setupEpubReader(bookPath)
            android.util.Log.d("ReadiumHelper", "EPUB文件处理完成")
            true
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "EPUB文件处理失败: ${e.message}", e)
            false
        }
    }
    ...
    private fun setupEpubReader(bookPath: String) {
        try {
            android.util.Log.d("ReadiumHelper", "开始解析EPUB文件: $bookPath")
            ...
            android.util.Log.d("ReadiumHelper", "EPUB解析成功，共 ${totalPages} 章")
            
            // 注意：UI 操作需要在主线程中执行，这里只设置数据
            // showReaderView() 将在主线程中调用
            
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "EPUB解析失败: ${e.message}", e)
    ...
    private fun parseEpubFile(bookPath: String): EpubContent {
        return try {
            ...
            android.util.Log.d("ReadiumHelper", "ZIP文件打开成功")
            
            // 读取容器信息
            val containerEntry = zipFile.getEntry("META-INF/container.xml")
            if (containerEntry == null) {
                android.util.Log.e("ReadiumHelper", "无法找到META-INF/container.xml")
                throw Exception("无法找到META-INF/container.xml")
            }
            android.util.Log.d("ReadiumHelper", "找到container.xml")
            
            val containerXml = zipFile.getInputStream(containerEntry).bufferedReader().use { it.readText() }
            android.util.Log.d("ReadiumHelper", "container.xml内容长度: ${containerXml.length}")
            
            val rootFilePath = extractRootFilePath(containerXml)
            android.util.Log.d("ReadiumHelper", "提取的根文件路径: $rootFilePath")
            
            // 读取内容清单
            val opfEntry = zipFile.getEntry(rootFilePath)
            if (opfEntry == null) {
                android.util.Log.e("ReadiumHelper", "无法找到OPF文件: $rootFilePath")
                throw Exception("无法找到OPF文件: $rootFilePath")
            }
            android.util.Log.d("ReadiumHelper", "找到OPF文件")
            
            val opfXml = zipFile.getInputStream(opfEntry).bufferedReader().use { it.readText() }
            android.util.Log.d("ReadiumHelper", "OPF文件内容长度: ${opfXml.length}")
            
            val chapters = parseOpfFile(opfXml, zipFile)
            android.util.Log.d("ReadiumHelper", "解析到 ${chapters.size} 个章节")
            
            // 解析元数据
            val metadata = parseMetadata(opfXml, zipFile)
            android.util.Log.d("ReadiumHelper", "解析到元数据: ${metadata?.title}")
            ...
            android.util.Log.e("ReadiumHelper", "EPUB文件解析失败: ${e.message}", e)
    ...
    private fun parseOpfFile(opfXml: String, zipFile: java.util.zip.ZipFile): List<Chapter> {
        ...
        try {
            ...
            android.util.Log.w("ReadiumHelper", "解析OPF文件时出错: ${e.message}")
        }
    ...
    private fun parseMetadata(opfXml: String, zipFile: java.util.zip.ZipFile): EpubMetadata? {
        return try {
            android.util.Log.d("ReadiumHelper", "开始解析EPUB元数据")
            ...
            val coverImageData = coverImagePath?.let { path ->
                try {
                    ...
                    android.util.Log.w("ReadiumHelper", "读取封面图片失败: ${e.message}")
                    null
                }
            }
            
            android.util.Log.d("ReadiumHelper", "元数据解析完成: 书名=$title, 作者=$author, 封面=$coverImagePath")
            ...
            android.util.Log.e("ReadiumHelper", "解析元数据失败: ${e.message}", e)
            null
        }
    }
        ...
        return try {
            ...
            if (coverIdMatch != null) {
                ...
                if (zipFile.getEntry(fullPath) != null) {
                    android.util.Log.d("ReadiumHelper", "通过ID找到封面: $fullPath")
                    return fullPath
                }
            }
            ...
            if (coverItemMatch != null) {
                ...
                val fullPath = if (coverPath.startsWith("/")) coverPath else "OEBPS/$coverPath"
                if (zipFile.getEntry(fullPath) != null) {
                    android.util.Log.d("ReadiumHelper", "通过文件名找到封面: $fullPath")
                    return fullPath
                }
            }
            ...
            for (match in imageMatches) {
                ...
                if (zipFile.getEntry(fullPath) != null) {
                    android.util.Log.d("ReadiumHelper", "找到图片文件作为封面: $fullPath")
                    return fullPath
                }
            }
            
            android.util.Log.d("ReadiumHelper", "未找到封面图片")
            null
            
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "查找封面图片失败: ${e.message}", e)
            null
        }
    }
    ...
    private fun showReaderView() {
        readerContainer?.let { container ->
            ...
            android.util.Log.d("ReadiumHelper", "阅读器视图已显示")
        }
    }
    ...
    private fun createReaderView(): android.view.View {
        android.util.Log.d("ReadiumHelper", "开始创建阅读器视图")
        ...
        val textView = android.widget.TextView(context).apply {
            ...
            android.util.Log.d("ReadiumHelper", "设置TextView内容，长度: ${currentContent.length}")
            text = currentContent
            ...
            android.util.Log.d("ReadiumHelper", "TextView设置完成")
        }
        
        scrollView.addView(textView)
        android.util.Log.d("ReadiumHelper", "阅读器视图创建完成")
        return scrollView
    }
    ...
    private fun getCurrentPageContent(): String {
        return try {
            android.util.Log.d("ReadiumHelper", "获取页面内容: currentBookPath=$currentBookPath, isBookLoaded=$isBookLoaded, currentPage=$currentPage, totalPages=$totalPages")
            
            if (currentBookPath != null && isBookLoaded) {
                // 尝试获取真实的 EPUB 内容
                val realContent = getRealEpubContent()
                if (realContent.isNotEmpty()) {
                    android.util.Log.d("ReadiumHelper", "返回真实EPUB内容，长度: ${realContent.length}")
                    return realContent
                }
                ...
                android.util.Log.d("ReadiumHelper", "返回模拟内容，长度: ${fallbackContent.length}")
                fallbackContent
            } else {
                android.util.Log.w("ReadiumHelper", "书籍未加载，返回提示信息")
                "请先加载书籍..."
            }
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "获取页面内容失败: ${e.message}", e)
            "获取页面内容失败: ${e.message}"
        }
    }
        ...
        return try {
            ...
            if (::epubContent.isInitialized && epubContent.chapters.isNotEmpty()) {
                ...
                if (chapterIndex >= 0 && chapterIndex < epubContent.chapters.size) {
                    ...
                    android.util.Log.d("ReadiumHelper", "获取第 ${currentPage} 页内容，章节标题: ${chapter.title ?: "未知标题"}")
                    ...
                    android.util.Log.w("ReadiumHelper", "页码超出范围: ${currentPage}, 总章节数: ${epubContent.chapters.size}")
                    ""
                }
            } else {
                android.util.Log.w("ReadiumHelper", "EPUB内容未初始化或为空")
                ""
            }
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "获取真实EPUB内容失败: ${e.message ?: "未知错误"}", e)
            ""
        }
    }
    ...
    private fun updateReaderContent() {
        try {
            android.util.Log.d("ReadiumHelper", "更新阅读器内容到第 ${currentPage} 页")
            ...
            readerContainer?.let { container ->
                ...
                if (textView != null) {
                    ...
                    android.util.Log.d("ReadiumHelper", "内容更新成功，长度: ${newContent.length}")
                } else {
                    // 如果找不到 TextView，回退到重新创建视图
                    android.util.Log.w("ReadiumHelper", "未找到现有TextView，重新创建视图")
                    updateReaderView()
                }
            }
        } catch (e: Exception) {
            android.util.Log.e("ReadiumHelper", "更新阅读器内容失败: ${e.message}", e)
            // 出错时回退到重新创建视图
            updateReaderView()
        }
    }
     ...
     * 检查书籍是否已加载
     */
    fun isBookLoaded(): Boolean = this.isBookLoaded
     ...
     * 获取当前配置
     */
    fun getCurrentConfig(): ReadiumConfig = config
