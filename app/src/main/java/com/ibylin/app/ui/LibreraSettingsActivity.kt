...
class LibreraSettingsActivity : AppCompatActivity() {
    ...
    @Inject
    lateinit var libreraConfig: LibreraConfig
    ...
    private fun initViews() {
        ...
        // 设置主题选择器
        val themes = libreraConfig.getAvailableThemes()
        ...
        // 设置翻页模式选择器
        val pageTurnModes = libreraConfig.getAvailablePageTurnModes()
    ...
    private fun setupListeners() {
        ...
        binding.seekBarFontSize.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    ...
                    libreraConfig.fontSize = fontSize
                }
            }
        ...
        binding.seekBarLineSpacing.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    ...
                    libreraConfig.lineSpacing = lineSpacing
                }
            }
            override fun onStartTrackingTouch(seekBar: SeekBar?) {}
            override fun onStopTrackingTouch(seekBar: SeekBar?) {}
        ...
        binding.spinnerTheme.setOnItemSelectedListener(object : android.widget.AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                libreraConfig.theme = selectedTheme
        ...
        binding.spinnerPageTurnMode.setOnItemSelectedListener(object : android.widget.AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: android.widget.AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                libreraConfig.pageTurnMode = selectedMode
            }
            override fun onNothingSelected(parent: android.widget.AdapterView<*>?) {}
        })
        ...
        binding.switchAutoScroll.setOnCheckedChangeListener { _, isChecked ->
            libreraConfig.autoScroll = isChecked
        }
        
        binding.switchNightMode.setOnCheckedChangeListener { _, isChecked ->
            libreraConfig.nightMode = isChecked
            applyNightMode(isChecked)
        }
        
        binding.switchFullscreen.setOnCheckedChangeListener { _, isChecked ->
            libreraConfig.fullscreen = isChecked
        }
    ...
    private fun loadCurrentSettings() {
        // 加载当前配置
        binding.seekBarFontSize.progress = ((libreraConfig.fontSize - 12f) / 2f).toInt()
        binding.tvFontSizeValue.text = "${libreraConfig.fontSize.toInt()}px"
        
        binding.seekBarLineSpacing.progress = ((libreraConfig.lineSpacing - 1.0f) / 0.1f).toInt()
        binding.tvLineSpacingValue.text = String.format("%.1f", libreraConfig.lineSpacing)
        
        // 设置主题选择器
        val themeIndex = libreraConfig.getAvailableThemes().indexOf(libreraConfig.theme)
        if (themeIndex >= 0) {
            binding.spinnerTheme.setSelection(themeIndex)
        }
        
        // 设置翻页模式选择器
        val pageTurnIndex = libreraConfig.getAvailablePageTurnModes().indexOf(libreraConfig.pageTurnMode)
        if (pageTurnIndex >= 0) {
            binding.spinnerPageTurnMode.setSelection(pageTurnIndex)
        }
        
        // 设置开关状态
        binding.switchAutoScroll.isChecked = libreraConfig.autoScroll
        binding.switchNightMode.isChecked = libreraConfig.nightMode
        binding.switchFullscreen.isChecked = libreraConfig.fullscreen
    }
